...
from utils.model_utils import METRICS, get_log_path
...
class Server:
    ...
    def aggregate_parameters(self):
        ...
        """
        # Calculate total samples across all selected users
        ...
        for user in self.selected_users:
            weight = user.train_samples / total_samples
            for server_param, user_param in zip(
                self.model.parameters(), user.model.parameters()
            ):
                server_param.data += weight * user_param.data.clone()

    def save_model(self):
        model_path = os.path.join(
            self.save_path, self.algorithm, "models", self.dataset
        )
    ...
    def load_model(self):
        model_path = os.path.join(
            self.save_path, "models", self.dataset, "server" + ".pt"
        )
    ...
    def select_users(self, round, num_users):
        ...
        """
        # Ensure num_users doesn't exceed total users
        num_users = min(num_users, len(self.users))
        # Randomly select users
        self.selected_users = np.random.choice(
            self.users, num_users, replace=False
        ).tolist()
    ...
    def evaluate_personalized_model(self, selected=True, save=True):
        ...
        test_loss = np.sum(
            [x * y for (x, y) in zip(test_num_samples, test_losses)]
        ).item() / np.sum(test_num_samples)
        if save:
            self.metrics["per_acc"].append(glob_acc)
            self.metrics["per_loss"].append(test_loss)
        print(
            "Average Global Accurancy = {:.4f}, Loss = {:.2f}.".format(
                glob_acc, test_loss
            )
        )
    ...
    def evaluate_ensemble(self, selected=True):
        ...
        for x, y in self.testloaderfull:
            ...
            test_acc += torch.sum(
                torch.argmax(target_logp, dim=1) == y
            )  # (torch.sum().item()
        ...
        print(
            "Average Global Accurancy = {:.4f}, Loss = {:.2f}.".format(test_acc, loss)
        )
    ...
    def evaluate(self, iter, save=True, selected=False):
        ...
        test_losses = [
            t.detach().cpu().numpy() for t in test_losses
        ]  # Error: RuntimeError: Can't call numpy() on Tensor that requires grad. #15
        glob_loss = np.sum(
            [x * y for (x, y) in zip(test_samples, test_losses)]
        ).item() / np.sum(test_samples)
        if save:
        ...
        self.logging.info(
            "Best Global Accurancy = {:.4f}, Loss = {:.2f}, Iter = {:}.".format(
                self.best_accu, self.best_loss, self.best_iter
            )
        )
