...
class User:
    ...
    def __init__(self, args, id, model, train_data, test_data, use_adam=False):
        ...
        self.trainloader = DataLoader(
            train_data, self.batch_size, shuffle=True, drop_last=True
        )
    ...
    def set_parameters(self, model, beta=1):
        ...
        for old_param, new_param in zip(self.model.parameters(), model.parameters()):
            old_param.data = beta * new_param.data.clone() + (1 - beta) * old_param.data
    ...
    def test(self):
        ...
        for x, y in self.testloaderfull:
            ...
            test_acc += (
                (torch.sum(torch.argmax(output, dim=1) == y)).detach().item()
            )  ## 修改detach
